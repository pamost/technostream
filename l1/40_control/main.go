...
func main() {
	...
	mm := map[string]string{"firstName": "Pavel", "lastName": "Mostovoy"}
	...
	if firstName, ok := mm["firstName"]; !ok {
		...
		println("firstName is Pavel")
	} else {
		println("firstName is not Pavel")
	}
	...
	switch {
	case mm["firstName"] == "Pavel":
		println("switch2 - Pavel")
	case mm["lastName"] == "Mostovoy":
		println("switch2 - Mostovoy")
	}

	// выход из цикла будучи внутри switch по метке
Loop:
	for key, val := range mm {
